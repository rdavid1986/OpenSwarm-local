...
export default function LongRunningTaskMonitor({
  ...
  const statusMeta: Record<TaskMonitorStatus, { label: string; color: string; bg: string }> = {
    idle: { label: 'Idle', color: SWARM_VSCODE_BLUE, bg: `${SWARM_VSCODE_BLUE}12` },
    running: { label: 'Running', color: SWARM_VSCODE_BLUE, bg: `${SWARM_VSCODE_BLUE}14` },
    waiting_approval: { label: 'Waiting approval', color: SWARM_VSCODE_BLUE, bg: `${SWARM_VSCODE_BLUE}14` },
    loading: { label: 'Loading', color: SWARM_VSCODE_BLUE, bg: `${SWARM_VSCODE_BLUE}14` },
    blocked: { label: 'Blocked', color: SWARM_VSCODE_BLUE, bg: `${SWARM_VSCODE_BLUE}14` },
    completed: { label: 'Completed', color: SWARM_VSCODE_BLUE, bg: `${SWARM_VSCODE_BLUE}12` },
    stopped: { label: 'Stopped', color: SWARM_VSCODE_BLUE, bg: `${SWARM_VSCODE_BLUE}12` },
  ...
  const metricToneColor = (tone?: TaskMonitorMetric['tone']) => {
    if (tone === 'info') return SWARM_VSCODE_BLUE;
    if (tone === 'warning') return SWARM_VSCODE_BLUE;
  ...
  return (
    <Box
      sx={{
        ...
        p: 0.9,
        borderRadius: 1.15,
        border: `1px solid ${SWARM_VSCODE_BLUE}35`,
        bgcolor: `${SWARM_VSCODE_BLUE}10`,
        boxShadow: `inset 0 1px 0 ${SWARM_VSCODE_BLUE}12`,
      }}
    >
      {isActive && (
        <LinearProgress
      ...
      <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 0.72, minWidth: 0 }}>
        <Box sx={{ flex: 1, minWidth: 0 }}>
          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.6, alignItems: 'center', mb: 0.55 }}>
            <Typography sx={{ color: c.text.primary, fontSize: '0.78rem', fontWeight: 850 }}>
              {title}
            </Typography>
            <Chip size="small" label={meta.label} sx={{ height: 20, color: meta.color, bgcolor: meta.bg, fontSize: '0.66rem', fontWeight: 700 }} />
            <Chip size="small" label={surfaceLabel} sx={{ height: 20, color: SWARM_VSCODE_BLUE, bgcolor: `${SWARM_VSCODE_BLUE}12`, border: `1px solid ${SWARM_VSCODE_BLUE}33`, fontSize: '0.66rem' }} />
            {isActive && startedAt && (
              <Chip
                size="small"
                icon={<AccessTimeIcon sx={{ fontSize: '0.78rem !important' }} />}
                label={`${formatElapsed(elapsedMs)} local`}
                sx={{ height: 20, color: SWARM_VSCODE_BLUE, bgcolor: `${SWARM_VSCODE_BLUE}0D`, border: `1px solid ${SWARM_VSCODE_BLUE}33`, fontSize: '0.66rem' }}
              />
            )}
          </Box>
      ...
      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5, mt: 0.75 }}>
        {builtMetrics.map((metric) => (
          <Chip
            ...
            sx={{ height: 19, color: metricToneColor(metric.tone), bgcolor: `${SWARM_VSCODE_BLUE}0D`, border: `1px solid ${SWARM_VSCODE_BLUE}26`, fontSize: '0.64rem' }}
          />
        ))}
      </Box>
    </Box>
