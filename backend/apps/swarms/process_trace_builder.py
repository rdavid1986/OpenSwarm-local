...
def build_sdd_orchestrator_process_trace_item(source: dict[str, Any]) -> dict[str, Any]:
    ...
    title_by_kind = {
        ...
        "sdd_implementation_candidate": "SDD implementation candidate",
        "sdd_verification_report": "SDD verification report",
        "sdd_evidence_trace": "SDD evidence trace",
        "sdd_delegation_decision": "SDD delegation decision",
        "sdd_spec_drift_report": "SDD spec drift report",
        "sdd_completion_gate": "SDD completion gate",
    }
    ...
    return build_process_trace_item(
        ...
        details={
            ...
            "source_contract_kind": data.get("source_contract_kind"),
            "patch_candidate": data.get("patch_candidate") if isinstance(data.get("patch_candidate"), dict) else {},
            "touched_files": data.get("touched_files") if isinstance(data.get("touched_files"), list) else [],
            "expected_diff_summary": data.get("expected_diff_summary"),
            "validation_commands": data.get("validation_commands") if isinstance(data.get("validation_commands"), list) else [],
            "rollback_plan": data.get("rollback_plan") if isinstance(data.get("rollback_plan"), list) else [],
            "materialization_request": data.get("materialization_request") if isinstance(data.get("materialization_request"), dict) else {},
            "materialization_decision": data.get("materialization_decision") if isinstance(data.get("materialization_decision"), dict) else {},
            "spec_compliance": data.get("spec_compliance"),
            ...
            "design_compliance": data.get("design_compliance"),
            "evidence_quality": data.get("evidence_quality"),
            "remaining_risks": data.get("remaining_risks") if isinstance(data.get("remaining_risks"), list) else [],
            "validation_refs": data.get("validation_refs") if isinstance(data.get("validation_refs"), list) else [],
            "materialization_refs": data.get("materialization_refs") if isinstance(data.get("materialization_refs"), list) else [],
            ...
            "input_contract_kind": data.get("input_contract_kind"),
            "expected_output_contract_kind": data.get("expected_output_contract_kind"),
            "context_packet_refs": data.get("context_packet_refs") if isinstance(data.get("context_packet_refs"), list) else [],
            "handoff_payload": data.get("handoff_payload") if isinstance(data.get("handoff_payload"), dict) else {},
            "previous_spec_hash": data.get("previous_spec_hash"),
            "current_spec_hash": data.get("current_spec_hash"),
            "drift_status": data.get("drift_status"),
            "drift_severity": data.get("drift_severity"),
            "changed_requirements": data.get("changed_requirements") if isinstance(data.get("changed_requirements"), list) else [],
            "changed_acceptance_criteria": data.get("changed_acceptance_criteria") if isinstance(data.get("changed_acceptance_criteria"), list) else [],
            "changed_design_refs": data.get("changed_design_refs") if isinstance(data.get("changed_design_refs"), list) else [],
            "return_stage": data.get("return_stage"),
            "change_control_decision": data.get("change_control_decision"),
            "can_continue_without_spec_update": data.get("can_continue_without_spec_update", False),
            "gate_status": data.get("gate_status"),
            ...
            "policy_matrix_required": data.get("policy_matrix_required", True),
            "can_materialize": data.get("can_materialize", False),
            "can_mark_verified": data.get("can_mark_verified", False),
            "can_mark_completed": data.get("can_mark_completed", False),
            "can_mark_complete": data.get("can_mark_complete", False),
